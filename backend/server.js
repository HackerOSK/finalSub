...
const genAI = new GoogleGenerativeAI("AIzaSyDo1sWjUVsVXs0XYfRoK16yvdACiq0Y8uY");
const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });

// Function to determine the area type
const fetchAreaType = async (location) => {
  const prompt = `Based on the location "${location}", determine if it is a developed, developing, or underdeveloped area. Provide only one of these three options as your answer.`;
  try {
    const result = await model.generateContent(prompt);
    return result.response.text.trim(); // Extract and return the AI's response
  } catch (error) {
