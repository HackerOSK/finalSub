...
const ApplyFund = () => {
  ...
  const fetchAreaType = async (location) => {
   ...
   const estimateFunds = async () => {
    ...
    const area = "developed";  // Default for now
    const bodyData = {
      location: plantData.cityName || 'pune',
      areaType: area,
      areaInAcres: plantData.areaInAcres || 10,
      initialCapacity: plantData.initialCapacity || 1000,
      targetCapacity: plantData.targetCapacity || 2000,
      recyclingDemand: plantData.recyclingDemand || 8000,
      recyclingCost: plantData.recyclingCost || 1
    };
  
    console.log('Data being sent to backend:', bodyData);  // Debugging log
    ...
    try {
      ...
      if (!response.ok) throw new Error('Failed to estimate funds');
